...
function Office(): JSX.Element {
  ...
  const [addCourseMutation] = useMutation<{ addCourse: CourseType }>(ADD_COURSE);
  const [updateOneCourseMutation] = useMutation<{ updateOneCourse: CourseType }>(
    UPDATE_COURSE,
  );
  const [deleteOneCourseMutation] = useMutation<{
    deleteOneCourse: { _id: string; message: string };
  }>(DELETE_ONE_COURSE);
